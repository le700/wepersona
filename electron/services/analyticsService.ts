...
class AnalyticsService {
  ...
  private async computeAggregateByCursor(sessionIds: string[], beginTimestamp = 0, endTimestamp = 0): Promise<any> {
    ...
    for (const sessionId of sessionIds) {
      ...
      await this.iterateSessionMessages(sessionId, (row) => {
        ...
        const hourWeekdayKey = `${hour}-${weekday}`
        ...
        aggregate.hourWeekdayMatrix[hourWeekdayKey] = (aggregate.hourWeekdayMatrix[hourWeekdayKey] || 0) + 1

        // 收集文本消息用于关键词分析
        if ((localType === 1 || localType === 244813135921) && row.content) {
          const content = String(row.content).trim()
          if (content.length > 0) {
            aggregate.textMessages.push(content)
          }
        }
  ...
  async getHeatmapData(): Promise<{ success: boolean; data?: HeatmapData; error?: string }> {
    try {
      const conn = await this.ensureConnected()
      if (!conn.success || !conn.cleanedWxid) return { success: false, error: conn.error }
      ...
      const d = result.data
      const xLabels = ['0时', '1时', '2时', '3时', '4时', '5时', '6时', '7时', '8时', '9时', '10时', '11时', '12时', '13时', '14时', '15时', '16时', '17时', '18时', '19时', '20时', '21时', '22时', '23时']
      const yLabels = ['周日', '周一', '周二', '周三', '周四', '周五', '周六']
      
      const heatmapData: Array<[number, number, number]> = []
      for (let hour = 0; hour < 24; hour++) {
        for (let weekday = 0; weekday < 7; weekday++) {
          const key = `${hour}-${weekday}`
          const count = d.hourWeekdayMatrix?.[key] || 0
          if (count > 0) {
  ...
  async getCalendarData(): Promise<{ success: boolean; data?: CalendarData[]; error?: string }> {
    try {
      const conn = await this.ensureConnected()
      if (!conn.success || !conn.cleanedWxid) return { success: false, error: conn.error }
      ...
      if (d.daily) {
        for (const [date, count] of Object.entries(d.daily)) {
          calendarData.push({ date, value: count as number })
        }
      }
  ...
  async getKeywordAnalysis(): Promise<{ success: boolean; data?: KeywordAnalysis; error?: string }> {
    try {
      const conn = await this.ensureConnected()
      if (!conn.success || !conn.cleanedWxid) return { success: false, error: conn.error }
      ...
      // 简单的词频统计 - 基于常用词汇和中文分词基础
      if (d.textMessages && Array.isArray(d.textMessages)) {
        const stopWords = new Set([
          '的', '了', '在', '是', '我', '有', '和', '就', '不', '人', '都', '一', '一个',
          '上', '也', '很', '到', '说', '要', '去', '你', '会', '着', '没有', '看', '好',
          '自己', '这', '那', '他', '她', '我们', '你们', '他们', '这个', '那个', '什么',
          '怎么', '为什么', '啊', '吧', '呢', '吗', '哦', '嗯', '哈', '呀', '啦', '嘛'
        ])

        for (const text of d.textMessages) {
          // 简单的基于正则的中文和英文单词提取
          // 提取2-4个汉字的组合
          ...
          if (englishMatches) {
            for (const word of englishMatches) {
              ...
              if (lowerWord.length > 2 && !stopWords.has(lowerWord)) {
                wordCountMap[lowerWord] = (wordCountMap[lowerWord] || 0) + 1
              }
            }
          }
        }
      ...
      // 转换为数组并排序
      const totalWords = Object.values(wordCountMap).reduce((a, b) => a + b, 0)
      const words = Object.entries(wordCountMap)
        .map(([word, count]) => ({
          word,
          count,
