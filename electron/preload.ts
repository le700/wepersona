...
contextBridge.exposeInMainWorld('electronAPI', {
  ...
  analytics: {
    ...
    getHeatmapData: () => ipcRenderer.invoke('analytics:getHeatmapData'),
    getCalendarData: () => ipcRenderer.invoke('analytics:getCalendarData'),
    getKeywordAnalysis: () => ipcRenderer.invoke('analytics:getKeywordAnalysis'),
  ...
  wechatBot: {
    ...
    getStatus: () => ipcRenderer.invoke('wechatbot:getStatus'),
    getConfig: () => ipcRenderer.invoke('wechatbot:getConfig'),
    saveConfig: (config: any) => ipcRenderer.invoke('wechatbot:saveConfig', config),
    onStatus: (callback: (status: any) => void) => {
      const listener = (_: any, status: any) => callback(status)
      ipcRenderer.on('wechatbot:status', listener)
      return () => ipcRenderer.removeListener('wechatbot:status', listener)
    },
    onQRCode: (callback: (qrCode: string) => void) => {
      const listener = (_: any, qrCode: string) => callback(qrCode)
      ipcRenderer.on('wechatbot:qrcode', listener)
      return () => ipcRenderer.removeListener('wechatbot:qrcode', listener)
    },
    onLogin: (callback: (userInfo: any) => void) => {
      const listener = (_: any, userInfo: any) => callback(userInfo)
      ipcRenderer.on('wechatbot:login', listener)
      return () => ipcRenderer.removeListener('wechatbot:login', listener)
    },
    onLogout: (callback: () => void) => {
      const listener = () => callback()
      ipcRenderer.on('wechatbot:logout', listener)
      return () => ipcRenderer.removeListener('wechatbot:logout', listener)
    },
    onError: (callback: (error: any) => void) => {
      const listener = (_: any, error: any) => callback(error)
      ipcRenderer.on('wechatbot:error', listener)
      return () => ipcRenderer.removeListener('wechatbot:error', listener)
    }
  }
})
